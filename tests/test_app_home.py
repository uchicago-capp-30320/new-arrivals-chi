...
def test_home_page_language_content(client):
    ...
    html_content = response.data.decode('utf-8')
    assert 'Welcome' in html_content
    assert 'Find essential support for legal, health, and food needs.' in html_content
    assert 'Please click below to begin:' in html_content

    response = client.get('/?lang=es')
    html_content = response.data.decode('utf-8')
    assert 'Bienvenido' in html_content
    assert 'Encuentra apoyo para tus necesidades legales, de salud o de comida.' in html_content
...
def test_home_page_button_links(client):
    """Test that buttons on the home page link correctly in English and Spanish."""
